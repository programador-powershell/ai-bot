/**
 * Contrapressão (aceite 4 do E3): cliente que para de ler leva 1013 e, ao
 * reconectar com resumeFrom, recebe o que faltou.
 *
 * O socket lento é DE VERDADE — pause() no socket do cliente para o kernel de
 * drenar — porque a cadeia inteira é física: o writeText espera o drain, a
 * fila do barramento enche, o assinante cai por atraso e a conexão fecha com
 * 1013. Mock de relógio provaria a coreografia, não a contrapressão.
 */
...
import { TOKEN_DE_TESTE, montarTransporte } from './teste-apoio.js'
...
describe('cliente atrasado (o Lagged → 1013 do oráculo)', () => {
  it('quem para de ler leva 1013 e o resumeFrom da reconexão recompõe o que faltou', async () => {
    // Folga curta para o atraso ser alcançável no teste; o linger folgado dá
    // tempo de o frame de close (que fica ATRÁS dos frames grandes no buffer)
    // chegar ao cliente quando ele voltar a ler.
    const transporte = await montarTransporte({
      folga: 8,
      transporte: { lingerMs: 10_000 },
    })
