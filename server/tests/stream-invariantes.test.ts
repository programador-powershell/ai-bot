/**
 * [Onda 2] Aceite do protocolo de stream SERVIDO PELO CHASSIS: as TRÊS
 * invariantes de ordem do stream.go (E3) como os MESMOS testes nomeados do
 * bridge (packages/harness-openbot-bridge/src/stream.test.ts), apontados para
 * o transporte novo — Bun.serve + WS nativo (src/stream/transporte.ts). O
 * clean-room continua provando o protocolo no runtime Node; ESTA suíte prova
 * que o chassis o serve de verdade.
 *
...
async function montar(
  ...args: Parameters<typeof montarTransporteDoChassi>
): Promise<TransporteDeTeste> {
  const transporte = await montarTransporteDoChassi(...args);
...
describe("hello (autenticação no primeiro frame, nunca na URL)", () => {
  ...
  it("origem de navegador fora da lista é 403 no upgrade — antes de qualquer frame", async () => {
    const transporte = await montar({
      transporte: { allowOrigins: ["http://localhost:1421"] },
    });
    await expect(
